...
- (void)hideCustomToast:(UIView *)toast;
- (void)customToastTimerDidFinish:(NSTimer *)timer;
...
- (void)makeCustomToast:(NSString *)message {
    [self makeCustomToast:message duration:CSToastDefaultDuration position:nil];
}

- (void)makeCustomToast:(NSString *)message duration:(NSTimeInterval)duration position:(id)position {
      ...
      tapCallback:(void(^)(void))tapCallback
{
    ...
    toast.accessibilityLabel = @"ToastView";
    for (UIView *subView in self.subviews) {
        if ([subView.accessibilityLabel isEqualToString:@"ToastView"]) {
    ...
    [UIView animateWithDuration:CSToastFadeDuration
                     ...
                     animations:^{
                         ...
                         NSTimer *timer = [NSTimer scheduledTimerWithTimeInterval:duration target:self selector:@selector(customToastTimerDidFinish:) userInfo:toast repeats:NO];
...
- (void)customToastTimerDidFinish:(NSTimer *)timer {
    [self hideCustomToast:(UIView *)timer.userInfo];
}
...
- (void)handleToastTapped:(UITapGestureRecognizer *)recognizer {
    ...
    [self hideCustomToast:recognizer.view];
}
...
- (void)makeToastActivity:(id)position {
    ...
    if (CSToastDisplayShadow) {
        activityView.layer.shadowColor = [[UIColor blackColor] colorWithAlphaComponent:CSToastOpacity].CGColor;
...
- (CGPoint)centerPointForPosition:(id)point withToast:(UIView *)toast {
    ...
    // default to bottom
    return CGPointMake(self.bounds.size.width/2, (self.bounds.size.height - (toast.frame.size.height / 2)) - 50);
}
...
- (UIView *)viewForMessage:(NSString *)message title:(NSString *)title image:(UIImage *)image {
    ...
    if (CSToastDisplayShadow) {
        wrapperView.layer.shadowColor = [[UIColor blackColor] colorWithAlphaComponent:CSToastOpacity].CGColor;
